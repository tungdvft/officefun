<template>
  <div class="container mx-auto bg-white rounded-xl shadow-lg overflow-hidden font-inter">
    <div class="p-6 space-y-8">
      <!-- Tiêu đề chính -->
      <h2 class="text-3xl font-bold text-center text-teal-700 mb-6">Chỉ Số Năng Lực Tự Nhiên</h2>
      ...
      <transition name="fade-slide">
        ...
        <div v-else-if="loading" class="text-center py-12">
          <div class="inline-flex items-center">
            ...
            <span class="text-gray-600">Đang phân tích chỉ số năng lực tự nhiên...</span>
          </div>
        </div>

        <!-- Hiển thị kết quả -->
        <div v-else-if="showResult" class="space-y-6">
          <!-- Hiển thị số Năng Lực Tự Nhiên và mô tả -->
          <div
            ...
            <div class="flex flex-col items-center">
              <div class="relative">
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                  <span class="text-5xl font-bold text-teal-700">{{ abilityNumber }}</span>
                </div>
              </div>
              <h3 class="text-2xl font-bold text-teal-800 mt-6">
                Chỉ Số Năng Lực Tự Nhiên {{ abilityNumber }}
              </h3>
              <p class="text-gray-600 mt-2 max-w-lg">{{ abilityData.description }}</p>
            </div>
          </div>
              ...
              >
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ abilityData.excellence }}</p>
                  </div>
                </div>
              </div>
              ...
              >
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ abilityData.path }}</p>
                  </div>
                </div>
              </div>
                ...
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <ul class="list-disc pl-5 mt-2 text-gray-600">
                      <li v-for="(advice, index) in abilityData.advice" :key="index">
                        {{ advice }}
                      </li>
                    </ul>
                  </div>
...
const description = 'Access to detailed natural ability number interpretations';
const {
  isLoading,
  errorMessage,
  errorType,
...
const showResult = computed(() => {
  return abilityData.value && abilityNumber.value !== null && !error.value;
});

// Tải dữ liệu Chỉ Số Năng Lực Tự Nhiên
const loadAbilityData = async () => {
  ...
  try {
    ...
    if (!abilityNumber.value || abilityNumber.value < 1 || abilityNumber.value > 9) {
      throw new Error(
        'Không thể tính toán Chỉ số Năng Lực Tự Nhiên. Vui lòng kiểm tra lại thông tin.'
      );
    }
    ...
    if (data) {
      ...
      throw new Error(
        `Không tìm thấy dữ liệu cho Chỉ số Năng Lực Tự Nhiên ${abilityNumber.value}`
      );
    }
  } catch (err) {
    console.error('[NaturalAbility] Error:', err.message);
    error.value = err.message;
    errorMessage.value = err.message;
    errorType.value = '';
    abilityData.value = {
      description: `Tạm thời chưa có dữ liệu chi tiết cho Chỉ số Năng Lực Tự Nhiên ${
        abilityNumber.value || 'không xác định'
      }`,
...
const initializeAuth = async () => {
  try {
    ...
    console.error('[NaturalAbility] Auth initialization error:', err);
