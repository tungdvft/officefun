        ...
        <div v-else-if="showResult" class="space-y-6">
          ...
          <div class="bg-gradient-to-r from-teal-50 to-blue-50 p-8 rounded-2xl border border-teal-100 shadow-sm text-center">
            <div class="flex flex-col items-center">
              ...
              <h3 class="text-2xl font-bold text-teal-800 mt-6">Chỉ Số Năng Lực Tự Nhiên {{ abilityNumber }}</h3>
                ...
                <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow md:col-span-2">
                  <div class="flex items-start mb-4">
                    ...
                    <div class="ml-4">
                      ...
                      <p class="text-gray-600 mt-2">{{ abilityData.description }}</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="flex items-start mb-4">
                    ...
                    <div class="ml-4">
                      ...
                      <ul class="list-disc pl-5 mt-2 text-gray-600">
                        <li v-for="(advice, index) in abilityData.advice" :key="index">{{ advice }}</li>
                      </ul>
                    </div>
                  </div>
                </div>
...
const loadAbilityData = async () => {
  ...
  try {
    ...
    if (!abilityNumber.value || abilityNumber.value < 1 || abilityNumber.value > 9) {
      throw new Error('Không thể tính toán Chỉ số Năng Lực Tự Nhiên. Vui lòng kiểm tra lại thông tin.');
    }
    ...
    if (data) {
      ...
      throw new Error(`Không tìm thấy dữ liệu cho Chỉ số Năng Lực Tự Nhiên ${abilityNumber.value}`);
    }
  } catch (err) {
    console.error('Lỗi trong loadAbilityData:', err);
    error.value = err.message;
    abilityData.value = {
      description: `Tạm thời chưa có dữ liệu chi tiết cho Chỉ số Năng Lực Tự Nhiên ${abilityNumber.value || 'không xác định'}`,
