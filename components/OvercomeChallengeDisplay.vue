<template>
  <div class="container mx-auto bg-white rounded-xl shadow-lg overflow-hidden font-inter">
    <div class="p-6 space-y-8">
      <!-- Tiêu đề chính -->
      <h2 class="text-3xl font-bold text-center text-teal-700 mb-6">Chỉ Số Vượt Khó</h2>
      ...
      <transition name="fade-slide">
        ...
        <div v-else-if="loading" class="text-center py-12">
          <div class="inline-flex items-center">
            ...
            <span class="text-gray-600">Đang phân tích chỉ số vượt khó...</span>
          </div>
        </div>

        <!-- Hiển thị kết quả -->
        <div v-else-if="showResult" class="space-y-6">
          <!-- Hiển thị số Vượt Khó và mô tả -->
          <div
          ...
          >
            <div class="flex flex-col items-center">
              ...
              <h3 class="text-2xl font-bold text-teal-800 mt-6">
                Chỉ Số Vượt Khó {{ challengeNumber }}
              </h3>
                ...
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <ul class="list-disc pl-5 mt-2 text-gray-600">
                      <li v-for="(advice, index) in challengeData.advice" :key="index">
                        {{ advice }}
                      </li>
                    </ul>
                  </div>
...
const description = 'Access to detailed overcome challenge number interpretations';
const {
  isLoading,
  errorMessage,
  errorType,
...
const showResult = computed(() => {
  return challengeData.value && challengeNumber.value !== null && !error.value;
});

// Tải dữ liệu Chỉ Số Vượt Khó
const loadChallengeData = async () => {
  ...
  try {
    ...
    if (!challengeNumber.value || challengeNumber.value < 1 || challengeNumber.value > 9) {
      throw new Error('Không thể tính toán Chỉ số Vượt Khó. Vui lòng kiểm tra lại thông tin.');
    }
    ...
    if (data) {
      ...
      throw new Error(`Không tìm thấy dữ liệu cho Chỉ số Vượt Khó ${challengeNumber.value}`);
    }
  } catch (err) {
    console.error('[OvercomeChallenge] Error:', err.message);
    error.value = err.message;
    errorMessage.value = err.message;
    errorType.value = '';
    challengeData.value = {
      description: `Tạm thời chưa có dữ liệu chi tiết cho Chỉ số Vượt Khó ${
        challengeNumber.value || 'không xác định'
      }`,
...
const initializeAuth = async () => {
  try {
    ...
    console.error('[OvercomeChallenge] Auth initialization error:', err);
