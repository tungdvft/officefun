...
<template>
  <div class="container mx-auto bg-white rounded-xl shadow-lg overflow-hidden font-inter">
    <div class="p-6 space-y-8">
      <h2 class="text-3xl font-bold text-center text-teal-700 mb-6">Chỉ Số Năng Lực Tiếp Cận</h2>
      ...
      <transition name="fade-slide">
        ...
        <div v-else-if="loading" class="text-center py-12">
          <div class="inline-flex items-center">
            ...
            <span class="text-gray-600">Đang phân tích chỉ số năng lực tiếp cận...</span>
          </div>
        </div>
        
        <!-- Hiển thị kết quả -->
        <div v-else-if="showResult" class="space-y-6">
          <!-- Hiển thị số Năng Lực Tiếp Cận -->
          <div
            ...
            <div class="flex flex-col items-center">
              <div class="relative">
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                  <span class="text-5xl font-bold text-teal-700">{{ capacityNumber }}</span>
                </div>
              </div>
              <h3 class="text-2xl font-bold text-teal-800 mt-6">
                Chỉ Số Năng Lực Tiếp Cận {{ capacityNumber }}
              </h3>
              <p class="text-gray-600 mt-2 max-w-lg">{{ capacityData.description }}</p>
            </div>
          </div>
              ...
              >
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ capacityData.description }}</p>
                  </div>
                </div>
              </div>
              ...
              >
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ capacityData.excellence }}</p>
                  </div>
                </div>
              </div>
              ...
              >
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ capacityData.path }}</p>
                  </div>
                </div>
              </div>
                ...
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <ul class="list-disc pl-5 mt-2 text-gray-600">
                      <li v-for="(advice, index) in capacityData.advice" :key="index">{{ advice }}</li>
                    </ul>
                  </div>
                </div>
              </div>
...
const description = 'Access to detailed approach capacity number interpretations';
const {
  isLoading,
  errorMessage,
  errorType,
...
const showResult = computed(() => {
  return capacityData.value && capacityNumber.value !== null && !error.value;
});

// Tải dữ liệu Chỉ Số Năng Lực Tiếp Cận
const loadCapacityData = async () => {
  ...
  try {
    ...
    if (!capacityNumber.value || capacityNumber.value < 1 || capacityNumber.value > 9) {
      throw new Error('Không thể tính toán Chỉ số Năng Lực Tiếp Cận. Vui lòng kiểm tra lại thông tin.');
    }
    ...
    if (data) {
      ...
      throw new Error(`Không tìm thấy dữ liệu cho Chỉ số Năng Lực Tiếp Cận ${capacityNumber.value}`);
    }
  } catch (err) {
    console.error('[ApproachCapacity] Error:', err.message);
    error.value = err.message;
    errorMessage.value = err.message;
    errorType.value = '';
    capacityData.value = {
      description: `Tạm thời chưa có dữ liệu chi tiết cho Chỉ số Năng Lực Tiếp Cận ${
        capacityNumber.value || 'không xác định'
      }`,
...
const initializeAuth = async () => {
  try {
    ...
    console.error('[ApproachCapacity] Auth initialization error:', err);
