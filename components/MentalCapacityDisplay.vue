<template>
  <div class="mental-capacity-display max-w-800px mx-auto p-5 font-inter">
    <h2 class="text-2xl font-bold text-center text-teal-800 mb-6">Chỉ số Năng Lực Tư Duy</h2>
    ...
    <div v-else-if="showResult" class="mt-6 animate-fadeIn">
      <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div class="flex items-center">
          ...
          <p class="text-gray-700 text-lg">Số Năng Lực Tư Duy: <span class="font-bold text-teal-600">{{ capacityNumber }}</span></p>
        </div>
      </div>
...
const loadCapacityData = async () => {
  ...
  try {
    ...
    if (!capacityNumber.value) {
      throw new Error('Không thể tính toán chỉ số Năng Lực Tư Duy');
    }
