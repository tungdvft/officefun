      ...
      <transition name="fade-slide">
        ...
        <div v-else-if="error" class="text-center py-12 bg-red-50 rounded-lg">
          ...
          <p class="text-gray-600 mt-1">
            Vui lòng nhập đầy đủ họ tên và ngày sinh hợp lệ (DD/MM/YYYY) để xem chỉ số thử thách sứ mệnh
          </p>
        </div>

        <!-- Hiển thị kết quả -->
        <div v-else-if="showResult" class="space-y-8">
...
const showResult = computed(() => {
  return data.value !== null && number.value !== null && !error.value;
});

// Xử lý lỗi dữ liệu đầu vào
const validateAndLoadData = () => {
  ...
  if (!props.birthDate || !/^\d{2}\/\d{2}\/\d{4}$/.test(props.birthDate)) {
    error.value = 'Ngày sinh không hợp lệ, vui lòng dùng định dạng DD/MM/YYYY';
    number.value = null;
    data.value = null;
    return;
  }
