<template>
  <div class="container mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
    <div class="p-6 space-y-8">
      <h2 class="text-3xl font-bold text-center text-teal-700 mb-6">Chỉ Số Năng Lực Tư Duy</h2>
      ...
      <transition name="fade-slide">
        ...
        <div v-else-if="loading" class="text-center py-12">
          <div class="inline-flex items-center">
            ...
            <span class="text-gray-600">Đang phân tích chỉ số năng lực tư duy...</span>
          </div>
        </div>
        
        <!-- Hiển thị kết quả -->
        <div v-else-if="showResult" class="space-y-6">
          <!-- Hiển thị số Năng Lực Tư Duy -->
          <div class="bg-gradient-to-r from-teal-50 to-blue-50 p-8 rounded-2xl border border-teal-100 shadow-sm text-center">
            <div class="flex flex-col items-center">
              ...
              <h3 class="text-2xl font-bold text-teal-800 mt-6">Chỉ Số Năng Lực Tư Duy {{ capacityNumber }}</h3>
...
const loadCapacityData = async () => {
  ...
  try {
    ...
    capacityNumber.value = calculateMentalCapacityNumber(props.fullName, props.birthDate);

    if (!capacityNumber.value || capacityNumber.value < 1 || capacityNumber.value > 9) {
      throw new Error('Không thể tính toán Chỉ số Năng Lực Tư Duy. Vui lòng kiểm tra lại thông tin.');
    }

    const data = mentalCapacityData.capacities.find(
    ...
    if (data) {
      ...
      throw new Error(`Không tìm thấy dữ liệu cho Chỉ số Năng Lực Tư Duy ${capacityNumber.value}`);
    }
  } catch (err) {
    console.error('Lỗi trong loadCapacityData:', err);
    error.value = err.message;
    capacityData.value = {
      description: `Tạm thời chưa có dữ liệu chi tiết cho Chỉ số Năng Lực Tư Duy ${capacityNumber.value || 'không xác định'}`,
