<template>
  <div class="bg-white rounded-xl shadow-lg p-4 sm:p-6">
    <div v-if="!mbtiData" class="text-red-600 text-center">
    ...
    <div v-else class="text-center">
      <p class="text-lg text-gray-700 mb-4">Bạn đã hoàn thành trắc nghiệm MBTI!</p>
...
console.log('Initial state:', { isCompleted: isCompleted.value, storeCompleted: store.mbtiCompleted }); // Debug
...
const nextQuestion = () => {
  ...
  if (currentQuestionIndex.value < questions.value.length - 1) {
    ...
    console.log('Completing MBTI quiz'); // Debug
    store.completeMbti();
    isCompleted.value = true;
    console.log('After complete:', { isCompleted: isCompleted.value, storeCompleted: store.mbtiCompleted }); // Debug
  }
};
...
const handleComplete = async () => {
  console.log('handleComplete called, redirecting to /xem/mbti-result'); // Debug
  console.log('Answers:', store.mbtiAnswers); // Debug
  if (Object.keys(store.mbtiAnswers).length < 40) {
    errorMessage.value = 'Vui lòng trả lời đủ 40 câu hỏi.';
