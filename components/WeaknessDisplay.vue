...
const showResult = computed(() => {
  return (
    Array.isArray(weaknessDataList.value) &&
    weaknessDataList.value.length > 0 &&
    Array.isArray(weaknessNumbers.value) &&
    weaknessNumbers.value.length > 0 &&
    !error.value
  );
});
...
const loadWeaknessData = async () => {
  ...
  try {
    ...
    if (!Array.isArray(weaknessNumbers.value) || weaknessNumbers.value.length === 0) {
    ...
    console.error('[WeaknessAnalysis] Error:', err.message);
