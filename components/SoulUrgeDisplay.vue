    ...
    <div class="p-6 space-y-8">
      ...
      <transition name="fade-slide">
        ...
        <div v-if="number && data" class="space-y-8">
          <!-- Hiển thị chỉ số linh hồn (không bảo vệ) -->
          <div class="bg-gradient-to-r from-teal-50 to-blue-50 p-8 rounded-2xl border border-teal-100 shadow-sm text-center">
            <div class="flex flex-col items-center">
              ...
              <h3 class="text-2xl font-bold text-teal-800 mt-6">Chỉ Số Linh Hồn Của Bạn</h3>
              <p class="text-gray-600 mt-2 max-w-lg">Số linh hồn: <span class="font-bold text-teal-600">{{ number }}</span></p>
            </div>
          </div>
        ...
        <div v-else class="text-center py-12 bg-red-50 rounded-lg">
          ...
          <p class="text-gray-600 mt-1">Không có dữ liệu chỉ số linh hồn.</p>
        </div>
      </transition>
    </div>
  </div>
...
onMounted(() => {
  ...
  if (props.birthDate && /^\d{2}\/\d{2}\/\d{4}$/.test(props.birthDate) && props.fullName) {
    initializeAuth();
