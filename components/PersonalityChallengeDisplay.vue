          ...
          >
            <div class="flex flex-col items-center">
              ...
              <h3 class="text-2xl font-bold text-teal-800 mt-6">
                Chỉ Số Thử Thách Nhân Cách {{ challengeNumber }}
              </h3>
...
import personalityChallengeData from '~/data/personalityChallengeData.json';
...
const description = 'Access to detailed personality challenge number interpretations';
const {
  isLoading,
  errorMessage,
  errorType,
...
// Tải dữ liệu Chỉ Số Thử Thách Nhân Cách
const loadChallengeData = async () => {
  ...
  try {
    ...
    if (!challengeNumber.value || challengeNumber.value < 1 || challengeNumber.value > 9) {
      throw new Error(
        'Không thể tính toán Chỉ số Thử Thách Nhân Cách. Vui lòng kiểm tra lại thông tin.'
      );
    }

    const data = personalityChallengeData.challenges.find(
    ...
    if (data) {
      ...
      throw new Error(
        `Không tìm thấy dữ liệu cho Chỉ số Thử Thách Nhân Cách ${challengeNumber.value}`
      );
    }
  } catch (err) {
    console.error('[PersonalityChallenge] Error:', err.message);
    error.value = err.message;
    errorMessage.value = err.message;
    errorType.value = '';
    challengeData.value = {
      description: `Tạm thời chưa có dữ liệu chi tiết cho Chỉ số Thử Thách Nhân Cách ${
...
const initializeAuth = async () => {
  try {
    ...
    console.error('[PersonalityChallenge] Auth initialization error:', err);
