<template>
  <div class="bg-white rounded-xl shadow-lg overflow-hidden">
    ...
    <div class="bg-gradient-to-r from-purple-700 to-pink-600 p-6 text-white">
      <h2 class="text-2xl sm:text-3xl font-bold mb-2">Trắc nghiệm tính cách DISC</h2>
      <p class="opacity-90">Khám phá phong cách hành vi của bạn qua 20 câu hỏi</p>
    </div>
    
    <!-- Error states -->
    <div v-if="!discData" class="p-6 text-center">
    ...
    <transition name="fade-scale" mode="out-in">
      <div v-if="isCompleted" class="p-6 sm:p-8 text-center">
        ...
        <p class="text-gray-600 mb-6 max-w-md mx-auto">
          Bạn đã hoàn thành trắc nghiệm DISC. Hãy khám phá kết quả chi tiết về phong cách hành vi của bạn.
        </p>
...
const isCompleted = ref(store.discCompleted);
...
const retryLoading = () => {
  ...
  setTimeout(() => {
    isLoading.value = false;
    questions.value = discData?.questions || [];
  }, 800);
};
...
const handleComplete = async () => {
  if (Object.keys(store.discAnswers).length < questions.value.length) {
