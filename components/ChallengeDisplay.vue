<template>
  <div class="container mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
    <div class="p-6 space-y-6">
      <!-- Nội dung chỉ số thử thách sứ mệnh -->
      <transition name="fade-slide">
        <div v-if="number && data" class="mt-6">
          <div class="space-y-6">
            <!-- Hiển thị chỉ số thử thách sứ mệnh -->
            <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
              <h3 class="text-xl font-bold text-teal-800 mb-4">Chỉ Số Thử Thách Sứ Mệnh Của Bạn</h3>
              <div class="flex items-center">
                ...
                <p class="text-gray-700 text-lg">Số thử thách sứ mệnh: <span class="font-bold text-teal-600">{{ number }}</span></p>
              </div>
            </div>
            <!-- Luận giải chi tiết -->
            <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div v-else class="text-center text-red-600">Không có dữ liệu chỉ số thử thách sứ mệnh.</div>
      </transition>
    </div>
  </div>
</template>
