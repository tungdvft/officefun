<template>
  <div class="bg-white rounded-xl shadow-xl p-6 border-l-4 border-teal-500 transition-all hover:shadow-2xl">
    ...
    <div v-else>
      ...
      <div class="space-y-4">
        ...
        <button 
          @click="getPlan" 
          :disabled="isLoading" 
          class="w-full bg-gradient-to-r from-teal-500 to-cyan-500 text-white px-6 py-3 rounded-lg hover:from-teal-600 hover:to-cyan-600 transition-all shadow-md flex items-center justify-center"
        >
          <svg v-if="isLoading" class="animate-spin w-5 h-5 mr-2" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none" />
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
          </svg>
          {{ isLoading ? 'Đang tạo kế hoạch...' : 'Lấy kế hoạch' }}
        </button>
      </div>
