...
<template>
  <div class="container mx-auto bg-white rounded-xl shadow-lg overflow-hidden font-inter">
    <div class="p-6 space-y-8">
      <h2 class="text-3xl font-bold text-center text-teal-700 mb-6">Chỉ Số Thái Độ Tiếp Cận</h2>
      ...
      <transition name="fade-slide">
        ...
        <div v-else-if="loading" class="text-center py-12">
          <div class="inline-flex items-center">
            ...
            <span class="text-gray-600">Đang phân tích chỉ số thái độ tiếp cận...</span>
          </div>
        </div>
        
        <!-- Hiển thị kết quả -->
        <div v-else-if="showResult" class="space-y-6">
          <!-- Hiển thị số Thái Độ Tiếp Cận -->
          <div
            ...
            <div class="flex flex-col items-center">
              <div class="relative">
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                  <span class="text-5xl font-bold text-teal-700">{{ attitudeNumber }}</span>
                </div>
              </div>
              <h3 class="text-2xl font-bold text-teal-800 mt-6">
                Chỉ Số Thái Độ Tiếp Cận {{ attitudeNumber }}
              </h3>
              <p class="text-gray-600 mt-2 max-w-lg">{{ attitudeData.description }}</p>
            </div>
          </div>
              ...
              >
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ attitudeData.description }}</p>
                  </div>
                </div>
              </div>
              ...
              >
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ attitudeData.excellence }}</p>
                  </div>
                </div>
              </div>
              ...
              >
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <p class="text-gray-600 mt-2">{{ attitudeData.path }}</p>
                  </div>
                </div>
              </div>
                ...
                <div class="flex items-start mb-4">
                  ...
                  <div class="ml-4">
                    ...
                    <ul class="list-disc pl-5 mt-2 text-gray-600">
                      <li v-for="(advice, index) in attitudeData.advice" :key="index">{{ advice }}</li>
                    </ul>
                  </div>
                </div>
              </div>
...
const attitudeNumber = ref(null);
const attitudeData = ref(null);
const loading = ref(false);
const error = ref(null);
const tokenCost = ref(5);
const description = 'Access to detailed interpretations of your Attitude Number, reflecting your approach to goals and challenges';
const {
  isLoading,
  errorMessage,
  errorType,
...
// Hàm tính Chỉ Số Thái Độ Tiếp Cận theo thần số học
const calculateApproachAttitudeNumber = (birthDate) => {
  try {
    const [day, month] = birthDate.split('/').map(Number);
    let sum = day + month;
    
    // Rút gọn tổng về một chữ số (1-9)
    while (sum > 9) {
      sum = String(sum)
        .split('')
    ...
    console.error('[calculateApproachAttitudeNumber] Error:', err);
    return null;
  }
};
...
const showResult = computed(() => {
  return attitudeData.value && attitudeNumber.value !== null && !error.value;
});

// Tải dữ liệu Chỉ Số Thái Độ Tiếp Cận
const loadAttitudeData = async () => {
  loading.value = true;
  error.value = null;
  attitudeNumber.value = null;
  ...
  try {
    ...
    attitudeNumber.value = calculateApproachAttitudeNumber(props.birthDate);

    if (!attitudeNumber.value || attitudeNumber.value < 1 || attitudeNumber.value > 9) {
      throw new Error('Không thể tính toán Chỉ số Thái Độ Tiếp Cận. Vui lòng kiểm tra lại thông tin.');
    }

    // Kiểm tra dữ liệu JSON
    if (!approachAttitudeData || !Array.isArray(approachAttitudeData.attitudes)) {
      throw new Error('Dữ liệu Chỉ số Thái Độ Tiếp Cận không khả dụng hoặc không đúng định dạng.');
    }

    const data = approachAttitudeData.attitudes.find(
      (item) => item.number === attitudeNumber.value
    );

    if (data) {
      attitudeData.value = data;
    } else {
      throw new Error(`Không tìm thấy dữ liệu cho Chỉ số Thái Độ Tiếp Cận ${attitudeNumber.value}`);
    }
  } catch (err) {
    console.error('[ApproachAttitude] Error:', err.message);
    error.value = err.message;
    errorMessage.value = err.message;
    errorType.value = '';
    attitudeData.value = {
      description: `Tạm thời chưa có dữ liệu chi tiết cho Chỉ số Thái Độ Tiếp Cận ${
        attitudeNumber.value || 'không xác định'
      }`,
...
const initializeAuth = async () => {
  try {
    ...
    console.error('[ApproachAttitude] Auth initialization error:', err);
