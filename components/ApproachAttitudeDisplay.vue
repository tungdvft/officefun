<template>
  <div class="approach-attitude-display max-w-800px mx-auto p-5 font-inter">
    <h2 class="text-2xl font-bold text-center text-teal-800 mb-6">Chỉ số Thái Độ Tiếp Cận</h2>
    ...
    <div v-else-if="showResult" class="mt-6 animate-fadeIn">
      <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        ...
        <div class="flex items-center">
          ...
          <p class="text-gray-700 text-lg">Số Thái Độ Tiếp Cận: <span class="font-bold text-teal-600">{{ attitudeNumber }}</span></p>
        </div>
      </div>
      ...
      <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-sm mt-6">
        ...
        <div class="prose prose-teal max-w-none space-y-3">
          <p class="text-gray-700"><strong>Mô tả:</strong> {{ attitudeData.description }}</p>
          <p class="text-gray-700"><strong>Lời khuyên:</strong></p>
          <ul class="list-disc pl-5">
            <li v-for="(advice, index) in attitudeData.advice" :key="index" class="text-gray-700">{{ advice }}</li>
          </ul>
        </div>
      </div>
    </div>
...
const loadAttitudeData = async () => {
  ...
  try {
    ...
    if (!attitudeNumber.value) {
      throw new Error('Không thể tính toán chỉ số Thái Độ Tiếp Cận');
    }
