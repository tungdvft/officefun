    ...
    <div v-if="numerologyData && numerologyData.numerology" class="max-w-4xl mx-auto space-y-6">
      ...
      <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <div class="p-6 sm:p-8 text-center bg-gradient-to-r from-blue-50 to-purple-50">
          ...
          <h2 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-1 mt-3">
            {{ numerologyData.numerology.name }}
          </h2>
        ...
        <div class="space-y-4">
          ...
          <div class="p-4 bg-blue-50 rounded-lg border-l-4 border-blue-400 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <p class="text-gray-700 pl-11">
              {{ numerologyData.numerology.profile.overview.core }}
            </p>
          </div>
          ...
          <div class="p-4 bg-green-50 rounded-lg border-l-4 border-green-400 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <p class="text-gray-700 pl-11">
              {{ numerologyData.numerology.profile.overview.nearFuture }}
            </p>
          </div>
          ...
          <div class="p-4 bg-orange-50 rounded-lg border-l-4 border-orange-400 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <p class="text-gray-700 pl-11">
              {{ numerologyData.numerology.profile.overview.challengesAndStrengths }}
            </p>
          </div>
          ...
          <div class="p-4 bg-purple-50 rounded-lg border-l-4 border-purple-400 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <p class="text-gray-700 pl-11">
              {{ numerologyData.numerology.profile.overview.longTermGoals }}
            </p>
          </div>
          ...
          <div class="p-4 bg-indigo-50 rounded-lg border-l-4 border-indigo-400 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <p class="text-gray-700 pl-11">
              {{ numerologyData.numerology.profile.overview.perception }}
            </p>
          </div>
          ...
          <div class="p-4 bg-pink-50 rounded-lg border-l-4 border-pink-400 shadow-sm hover:shadow-md transition-shadow duration-200">
            ...
            <p class="text-gray-700 pl-11">
              {{ numerologyData.numerology.profile.overview.tools }}
            </p>
          </div>
        </div>
      </div>
...
// State
const name = ref(userStore.fullname || "");
const birthDate = ref(userStore.birthdate || "");
const numerologyData = ref(userStore.numerologyData || null);
...
const fetchNumerology = async () => {
  ...
  try {
    ...
    if (data) {
      ...
      userStore.fetchNumerology({
        fullname: name.value,
        birthdate: birthDate.value,
      });
      userStore.numerologyData = data; // Cập nhật numerologyData trong store
    } else {
...
// Load dữ liệu từ store khi vào trang
onMounted(() => {
  if (userStore.fullname && userStore.birthdate) {
    name.value = userStore.fullname;
    birthDate.value = userStore.birthdate;
  }
  if (userStore.numerologyData) {
...
/* Nền không gian vũ trụ */
.space-background {
  background-image: url("/numerology-background.jpg");
...
.glow-number {
  text-shadow: 0 0 20px rgba(255, 255, 255, 0.9), 0 0 40px rgba(138, 43, 226, 0.8),
    0 0 60px rgba(75, 0, 130, 0.7), 0 0 80px rgba(0, 191, 255, 0.5);
  color: white;
  font-size: 6rem;
  font-weight: 900;
  position: absolute;
